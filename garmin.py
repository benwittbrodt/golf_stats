...
class GarminDataExport:
    ...
    def __init__(self, input) -> None:
        ...
        with open(input) as file:
            ...
            (cards_df, courses_df, holes_df) = self.process_scorecard(
                golf_raw["details"]
            )  # always one of these
            shots_df = self.process_shots(golf_raw["shotDetails"])
            last_10_stats = self.process_last10_stats(
                golf_raw["last10DataStats"]
            )  # strokes gained is nested here
            (last_10_drive, drive_shot_disp_df) = self.process_last10_drives(
                golf_raw["last10DataDrive"]
            )
            (
                last_10_approach,
                appr_shot_orientation_df,
                dist_range_insights_df,
                club_insights_df,
            ) = self.process_last10_approach(golf_raw["last10DataApproach"])
            last_10_chip, chip_shot_orientation_df = self.process_last10_chip(
                golf_raw["last10DataChip"]
            )
    ...
    def process_clubs(self, clubs):
        ...
        norm_stats = (
            pd.json_normalize(club_stats)
            .dropna(how="all")
            .drop(columns="lastModifiedTime")
        )
        norm_stats["id"] = norm_stats["id"].astype("int64")
        clubs_df = clubs_df.drop(columns="clubStats")
        return pd.merge(
            clubs_df, norm_stats, on="id", suffixes=("_clubs", "_club_stats")
        )
    ...
    # an array of shot Details, which then is the hole + all the shots
    def process_shots(self, shot_details):
...
if __name__ == "__main__":
    ...
    general.add_argument(
        "-i",
        "--input",
        default="golf-export.json",
        help="The input file to load and turn into CSVs",
    )
    general.add_argument(
        "-o",
        "--output",
